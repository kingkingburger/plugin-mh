---
name: daily-report
description: Source-agnostic daily work report generator. Use when the user wants a detailed end-of-day or period-based work report from conversation logs, agent logs, git activity, local notes, terminal history, or explicitly provided evidence sources. Trigger on "daily-report", "daily report", "work report", "작업 보고서", "일일 보고서", "오늘 대화 요약", "오늘 작업 정리", "하루 작업 보고서", "Codex 대화 전체 요약", "Claude 로그 요약", "로그 기반 보고서", "하루종일 한 일 정리". Produces an auditable Markdown report with timeline, project breakdown, decisions, failures, resolutions, follow-ups, and evidence sources. Does not write memory or harness notes unless the user explicitly asks.
version: 1.0.0
---

# daily-report: Source-Agnostic Work Report

하루 또는 지정 기간의 작업 흔적을 여러 소스에서 수집해 감사 가능한 Markdown 작업 보고서로 정리한다. 이 스킬의 핵심은 특정 도구(Codex, Claude, Cursor 등)가 아니라 **증거 소스 기반 보고서 작성**이다.
