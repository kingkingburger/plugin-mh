...
version: 1.0.0
description: >
  OpenAI의 하네스 엔지니어링 개념을 기반으로 기획, 문서작업, 디자인, 리서치, 운영,
  개발 같은 다양한 작업 표면의 문서 체계와 에이전트 팀 하네스를 구축한다.
  인터뷰 → 작업 표면 분석 → 팀 아키텍처 설계 → 문서/에이전트/스킬/오케스트레이터 생성으로
  반복 가능한 작업 시스템을 구성한다. 새 프로젝트 부트스트랩, 기존 자료 보완, 에이전트 팀 설계/점검 모두 지원.
  Trigger on "harness", "하네스", "문서 체계", "엔지니어링 문서", "프로젝트 구조", "기획", "문서작업", "디자인",
  "docs 구조", "harness engineering", "하네스 엔지니어링", "에이전트 팀", "오케스트레이터",
  "하네스 구성", "하네스 점검", "에이전트/스킬 동기화".
allowed-tools:
  - Agent
  - AskUserQuestion
  - Read
...
# Harness Engineering — Work Surface, Agent Team & Documentation Harness

기획, 문서작업, 디자인, 리서치, 운영, 개발 같은 다양한 작업 표면의 문서 체계와 에이전트 팀 구성을 **인터뷰 → 작업 표면 분석 → 팀 아키텍처 설계 → 문서/에이전트/스킬/오케스트레이터 생성**으로 한번에 구축하는 스킬.
...
인터뷰 답변 + 작업 표면 분석 결과를 context로 전달하여 **필요한 에이전트 그룹을 병렬 실행**한다. 개발 프로젝트가 아니면 아래 개발 중심 산출물을 그대로 만들지 말고 `references/work-surface-patterns.md`의 해당 표면 산출물로 치환한다.
