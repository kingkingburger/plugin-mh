...
plugin-mh의 24개 스킬 + 2개 에이전트를 OpenAI Codex CLI 슬래시 커맨드로 변환해둔 어댑터.
...
| Codex CLI | `codex/prompts/` 를 `~/.codex/prompts/` 에 링크 | 슬래시 커맨드 26개 추가 |
...
Codex CLI에서 `/` 입력 시 26개의 plugin-mh 명령어가 보여야 한다.
...
```
codex/
├── AGENTS.md                ← 26개 명령어 카탈로그 + 자연어 트리거 매핑
├── README.md                ← 본 파일
...
| **카탈로그** | [`./AGENTS.md`](./AGENTS.md) | 26개 슬래시 커맨드 카탈로그 + 자연어 트리거 매핑 |
