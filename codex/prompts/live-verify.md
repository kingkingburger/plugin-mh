# /live-verify — Live Verify: 실제 제품을 유저처럼 조작하여 E2E 검증

작업 계획에서 검증 기준을 설정하고(Phase 1), 구현 후 실제 제품을 브라우저/CLI/API로 조작하여 검증한다(Phase 2).
...
1. **실제 제품을 조작한다.** 단위 테스트/코드 리뷰가 아닌, 로컬에서 띄운 제품을 직접 클릭/입력/확인한다.
2. **모든 질문은 명확한 옵션과 함께 제시한다.** 열린 질문 대신 구체적 옵션을 제시한다.
...
**감지된 대상 타입이 맞나요?**
1. **{감지된 타입}** — {감지 근거 요약} (추천)
2. **웹 (Playwright MCP)** — 브라우저에서 클릭/입력으로 검증
3. **CLI (Bash)** — 명령어 실행 + 출력 비교로 검증
4. **API (curl)** — HTTP 요청 + 응답 비교로 검증
...
- 웹: "로그인 버튼 클릭" → Playwright MCP의 click 도구 호출로 직접 변환 가능해야 함
- CLI: "help 명령어 실행" → `Bash`로 직접 실행 가능해야 함
- API: "POST /api/users 호출" → `curl` 명령어로 직접 변환 가능해야 함

#### 1-3: 사용자 확인

생성된 시나리오 목록을 사용자에게 제시하세요:

사용자에게 다음 질문을 제시하고 번호 또는 라벨로 답하게 하세요:

**생성된 검증 시나리오 {N}건을 확인해주세요. 수정이 필요한가요?**
1. **좋습니다, 저장해주세요** — 시나리오를 그대로 저장하고 Phase 1을 완료합니다
2. **시나리오 추가 필요** — 빠진 시나리오가 있어서 추가하고 싶습니다
3. **시나리오 수정 필요** — 기존 시나리오의 단계나 기대결과를 수정하고 싶습니다
4. **시나리오 삭제 필요** — 불필요한 시나리오를 제거하고 싶습니다
...
2. 여러 파일이 있으면 사용자에게 선택 요청:

사용자에게 다음 질문을 제시하고 번호 또는 라벨로 답하게 하세요:

**검증할 시나리오 파일을 선택해주세요.**
1. **{최신 파일명}** — {생성일} / {시나리오 수}건 (추천)
2. **{이전 파일명}** — {생성일} / {시나리오 수}건
...
2. 실행 중이 아니면 사용자에게 확인:

사용자에게 다음 질문을 제시하고 번호 또는 라벨로 답하게 하세요:

**서버가 localhost:{port}에서 실행 중이지 않습니다. 어떻게 할까요?**
1. **서버 시작 명령어 알려주기** — 서버를 시작할 명령어를 알려주시면 백그라운드로 실행합니다
2. **이미 실행 중 (다른 포트)** — 포트 번호를 알려주시면 변경합니다
3. **나중에 하겠습니다** — Phase 2를 중단합니다
...
##### 웹 시나리오 실행 (Playwright MCP)

각 단계를 Playwright MCP의 navigate / click / snapshot 도구를 사용:

1. Playwright MCP를 통해 페이지 이동 (navigate)
2. Playwright MCP를 통해 현재 상태 확인 (snapshot — 접근성 트리)
3. Playwright MCP를 통해 유저 행동 수행 (click / fill_form / type)
4. Playwright MCP를 통해 결과 상태 확인 (snapshot)
5. Playwright MCP를 통해 증거 스크린샷 촬영 (take_screenshot)
...
검증 과정에서 아래 상황이 발생하면 해당 커맨드를 안내한다:
...
---
## 참조 파일 위치
이 프롬프트가 언급하는 `references/...` 파일은 plugin-mh 저장소의 `skills/live-verify/references/` 에 있다.
