...
Claude Code plugin with 21 custom skills for thinking, deciding, and building smarter.
...
**4단계 근본 원인 분석으로 체계적으로 디버깅.**

> 핵심 원칙: 근본 원인을 찾기 전에 수정을 시도하지 않는다. 증상 수정은 실패다.

**4 Phases:**
1. **근본 원인 조사** — 에러 읽기, 재현, 최근 변경 확인, 증거 수집
2. **패턴 분석** — 작동하는 유사 코드 찾기, 차이점 비교
3. **가설 검증** — 단일 가설 수립, 최소 변경으로 테스트
4. **구현** — 실패하는 테스트 → 단일 수정 → 검증

**3회 이상 수정 실패 = 아키텍처 문제.** 멈추고 근본적으로 재검토한다.

**Trigger:** `디버깅`, `버그 찾기`, `왜 안 돼`, `root cause`, `원인 분석`, `debug this`, `트러블슈팅`

```bash
# 예시
User: "디버깅 - 로그인할 때 500 에러가 나"
User: "왜 안 돼 - 테스트가 로컬에서만 실패해"
```
