...
`$ARGUMENTS`에 담긴 계획이나 설계를 바로 구현하지 말고, 결정 트리가 닫힐 때까지 짧고 날카로운 질문으로 압박한다. 목적은 큰 문서 시스템을 만드는 것이 아니라, 구현 전에 위험한 가정과 선택지를 빠르게 드러내는 것이다.
...
- 각 option description에는 결과와 trade-off를 한 문장으로 적는다.
- 답변을 받은 뒤 다음 질문 배치로 이어간다.
