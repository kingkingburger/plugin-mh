# ~/.codex/AGENTS.md — 개인 글로벌 가이드 (원민호)

> 이 파일은 `~/.codex/AGENTS.md` 로 복사하면 Codex가 모든 프로젝트에서 본 가이드를 자동 로드한다.
> 프로젝트별 AGENTS.md가 있으면 그것이 우선하고, 본 파일은 보조로 작용한다.
>
> **설치**: `cp codex/user-global-AGENTS.md ~/.codex/AGENTS.md`
> 또는 Windows: `Copy-Item codex/user-global-AGENTS.md $env:USERPROFILE\.codex\AGENTS.md`

---

## 사용자 프로필

- **이름**: 원민호
- **역할**: 소프트웨어 엔지니어
- **환경**: Windows, PowerShell, Git Bash
- **선호 언어**: Python (with `uv`, `ty`), TypeScript (with `bun`)
- **응답 언어**: 한국어 (기술 용어 / 코드 식별자는 원문 유지)

## 소통 / 업무 스타일

- 간결하고 핵심만 전달 — 단, 필요한 곳엔 상세한 배경 / 이유 함께
- 큰 작업은 단계별로 확인받으며 진행
- 방향이 명확하면 자율적으로 끝까지 진행하고 결과만 보고
- 구현 결과가 달라지는 모호함은 질문한다. 그 외에는 합리적으로 가정하고 진행한 뒤 가정을 보고한다.

## 응답 규칙

- 답변 시 최대한 서문으로 답변한다.
- 작업 완료 후 생성 / 변경한 파일은 **전체 경로** 로 출력
- 새 개인 정보 / 선호도 / 프로젝트 패턴 발견 시 메모리 업데이트 제안
- 코드 / 식별자는 원문, 설명은 한국어
- 한국어 정서법 준수 — 특수 문자(ä, ö, ü, ç, ñ 등)를 ASCII로 치환 금지

## 즐겨 쓰는 도구 (있을 때만)

- **AskUserQuestion** (Claude 한정) — 질문할 땐 반드시. Codex에서는 자연어 번호 옵션으로 대체.
- **Tasks / TaskCreate** — 여러 단계 작업 추적
- **Explore agent** — 정보 파악 / 코드베이스 탐색

## 셸 / 터미널 선호

- Windows에서도 PowerShell과 Git Bash를 모두 자주 사용한다.
- Codex 네이티브 Windows 세션에서는 bash / git / rg 같은 셸 명령을 `mcp__git_bash.run` 으로 먼저 실행한다.
- 기본 `bash` 또는 `exec_command` 의 `shell: "bash"` 가 WSL 경로로 잡혀 실패할 수 있으므로 먼저 시도하지 않는다.
- WSL bash 경로 실패가 발생해도 사용자에게 반복 복구 문구를 출력하지 말고, 조용히 Git Bash MCP / 명시적 Git Bash 경로 / 필요한 경우 PowerShell로 전환한다.
- 검증/설치 스크립트는 가능하면 PowerShell용 `.ps1`과 Git Bash에서 실행 가능한 `.sh` 경로를 함께 유지한다.
- 셸별 차이가 있는 명령은 한쪽만 검증했다고 단정하지 말고, 실제 사용 셸 기준으로 재확인한다.

## 언어별 기본 검증

| 언어 | 우선 도구 |
|------|-----------|
| TypeScript | `bun`, `bunx tsgo --noEmit` |
| Python | `uv`, `uv run ty check`, `pytest` |
| Rust | `cargo fmt`, `cargo clippy`, `cargo test` |

---

## plugin-mh 슬래시 커맨드 활용

다음 27개 명령어가 `~/.codex/prompts/` 에 설치되어 있다면 사용자 자연어 의도에 맞춰 자동 활용:

### 사고 정리 (Clarification)
- 모호한 요구 → `/vague`
- 전략 사각지대 → `/unknown`
- 관점 전환 → `/metamedium`
- 어떤 것일지 모르겠음 → `/clarify`
- 목표 상향 → `/moonshot`
- 경쟁형 장기 목표 / Judge Packet → `/deep-goal-council`

### 의사결정 / 리뷰
- 기술 선택 (A vs B) → `/tech-decision`
- 다관점 토론 → `/agent-arena`
- 문서 리뷰 → `/expert-review`
- 단독 코드 리뷰 → `/code-review`
- 체이닝 리뷰 → `/review-loop`

### 개발
- 테스트 주도 → `/tdd`
- 프로젝트 문서 체계 → `/harness`
- 하네스/스킬 꼬임·과도한 탐색 감사 → `/find-pulp`
- 자동 커밋 → `/auto-commit`
- E2E 검증 → `/live-verify`
- AI 슬롭 코드 정리 → `/ai-slop-cleaner`

### 스펙 / 실행
- 깊이 있는 스펙 → `/ouroboros`
- 계획 실행 → `/ouroboros-run`

### 세션
- 경량 마무리 → `/closing-lite`
- 풀 마무리 → `/session-closing`
- 일일 작업 보고서 → `/daily-report`
- 지식 큐레이션(메모리 누적) → `/knowledge-curator`

### 개인 계획
- 6계층 인생 계획 → `/life-plan`

### 콘텐츠
- YouTube 요약 → `/youtube-digest`
- YouTube 슬라이드 캡쳐 → `/youtube-slides`

### 메타
- 스킬 관리 → `/skill-manage` (plugin-mh 저장소 작업 시만)

---

## 자동 라우팅 트리거

사용자 입력 패턴에서 다음이 보이면 해당 명령어를 우선 제안 또는 실행:

| 입력 패턴 | 제안 명령어 |
|----------|-------------|
| "어떻게", "고민", "뭐가 나을까" + 비교 대상 2개+ | `/tech-decision` |
| "모호", "막연", "뭘 원하는지", "scope" | `/vague` 또는 `/clarify` |
| "blind spot", "사각지대", "가정 점검" | `/unknown` |
| "다른 방법", "관점 전환", "diminishing returns" | `/metamedium` |
| "10x", "더 큰 목표", "stretch" | `/moonshot` |
| "목표가 작", "단기적", "여러 경쟁 팀", "Judge Packet" | `/deep-goal-council` |
| "테스트 먼저", "TDD", "RED GREEN" | `/tdd` |
| "하네스 꼬임", "스킬 꼬임", "규칙 충돌", "과도한 탐색", "컨텍스트 과잉" | `/find-pulp` |
| "코드 리뷰", "review my code" | `/code-review` |
| "리뷰 루프", "고치고 다시" | `/review-loop` |
| "실제로 동작", "E2E", "브라우저로 테스트" | `/live-verify` |
| "AI 슬롭", "deslop", "슬롭 정리" | `/ai-slop-cleaner` |
| "커밋해" 단독 | `/auto-commit` |
| "정리하고 끝", "wrap up", "세션 마무리" | `/closing-lite` 또는 `/session-closing` |
| "작업 보고서", "일일 보고서", "오늘 작업 정리" | `/daily-report` |
| "인생 계획", "1년 방향", "오늘 미션" | `/life-plan` |

---

## 일반 작업 원칙

### Kapathy 핵심 가드 — 작업 전 최우선 적용
코드 작성, 버그 수정, 리팩터링, 코드 리뷰, 구현 계획 실행, 저장소 변경처럼 비용이 생길 수 있는 작업을 맡으면 가장 먼저 `~/.codex/skills/kapathy/SKILL.md`의 원칙을 적용한다. 사용자가 `kapathy` 또는 `karpathy`에게 조언을 구하면 전역 에이전트 `kapathy`의 판단 방식으로 답한다.

핵심은 네 가지다. 첫째, 가정한 채 달리지 말고 모호함, 충돌, 선택지, tradeoff를 드러낸다. 둘째, 현재 문제를 푸는 가장 작은 해법을 고르고 요청받지 않은 기능과 추상화를 만들지 않는다. 셋째, 모든 변경 줄이 사용자의 요청이나 그 요청으로 생긴 정리에 직접 연결되게 하며 drive-by refactor를 섞지 않는다. 넷째, 작업 완료 전 관찰 가능한 success criteria와 검증 증거를 세운다.

이 원칙은 실행을 늦추기 위한 의식이 아니다. 단순하고 명확한 작업은 짧게 적용하고 바로 처리하되, 구현 결과가 달라지는 모호함이나 위험한 범위 확장이 보이면 먼저 좁히고 반박한다.

### 모호함은 즉시 해소
요청에 모호한 표현이 있고 선택에 따라 구현 결과가 달라지면 코드 작성 전에 `/vague` 또는 자연어 번호 옵션으로 명확화한다. 실행 결과가 크게 달라지지 않는 세부사항은 합리적으로 가정하고 진행한 뒤 가정을 보고한다.

### 검증 없이 마무리 금지
구현 후 변경 표면에 맞는 검증을 실행한다. UI/E2E 변경은 `/live-verify` 수준의 실제 표면 검증이 필요하고, 위험한 코드 변경은 `/code-review` 또는 별도 리뷰 패스를 붙인다. "동작할 것 같다"는 검증이 아니다.

### 세션 종료 시 메모리 누적
사용자가 세션 마무리나 메모리 누적을 요청했고 큰 변화 / 새로 알게 된 패턴 / 사용자 선호가 있으면 `/closing-lite` 로 메모리에 한 줄 누적한다.
하루 작업을 감사 가능한 문서로 남겨야 하면 `/daily-report` 로 별도 Markdown 보고서를 만든다.

### 한국어 정서법
모든 한국어 텍스트는 정확한 정서법 준수. 영어 / 식별자는 원문 유지.

---

## 커밋 메시지 컨벤션

- 한국어
- 접두사: `추가:`, `수정:`, `삭제:`, `개선:`
- 형식: `<접두사> <한 줄 요약>` + 빈 줄 + 상세 내용
- Co-Authored-By: AI 협업 시 포함
- `git add -A` 대신 변경 파일 명시적으로 add
- `--no-verify` 금지 (명시 허락 없는 한)

---

## 위임 / 에이전트 호출

- 단순 검색은 직접 도구 호출 (Glob / Grep / Read)
- 복잡한 멀티스텝 조사 → 서브 에이전트 위임
- 내가 작성한 코드는 Codex가 1차 검증하고, 큰 변경이나 위험한 변경은 별도 에이전트 / 별도 패스로 재검증
- 토큰 큰 작업은 백그라운드 옵션 활용

---

## 참고

이 가이드는 plugin-mh 저장소의 [codex/AGENTS.md](https://github.com/kingkingburger/plugin-mh/blob/master/codex/AGENTS.md) 와 [codex/template/AGENTS.md](https://github.com/kingkingburger/plugin-mh/blob/master/codex/template/AGENTS.md) 의 요약본이다. 상세 사용법은 원본 문서 참조.
