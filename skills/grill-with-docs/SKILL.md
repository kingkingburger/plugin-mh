...
description: Stress-test a plan against the existing domain model, glossary, ADRs, and code before implementation. Trigger on "grill with docs", "grill-with-docs", "도메인 문서 기준으로 점검", "문서랑 맞춰 질문", "ADR까지 정리". Use when planning questions should also refine CONTEXT.md and ADR documentation as decisions crystallize.
version: 1.0.0
allowed-tools:
  - AskUserQuestion
  - Read
...
계획을 집요하게 질문하되, 질문의 기준을 저장소의 기존 언어와 결정 기록에 둔다. 사용자의 답변으로 도메인 용어가 확정되면 `CONTEXT.md`에 반영하고, 되돌리기 어려운 trade-off가 확정되면 ADR 작성을 제안한다.
...
AskUserQuestion:
  questions:
    - question: "{문서/코드 사실과 연결된 구체 질문}"
      header: "{짧은 라벨}"
      options:
        - label: "{추천 선택지} (Recommended)"
          description: "{도메인 언어, 구현, 문서에 미치는 영향}"
        - label: "{대안}"
          description: "{대안의 영향}"
```
