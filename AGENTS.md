...
plugin-mh는 MH의 커스텀 Claude Code 플러그인이다. 25개 스킬 + 2개 에이전트를 제공하며, 변환 어댑터(`codex/`) 를 통해 OpenAI Codex CLI에서도 동일한 기능을 27개 슬래시 커맨드로 사용할 수 있다.
`guardrails/` 는 TypeScript, Rust, Python 및 공통 소프트웨어 공학 법칙을 에이전트 행동 규칙으로 고정하는 하네스 레이어다.
...
빌드 시스템 없음. 검증은 다음 명령과 체크로 충분:

| 검증 | 명령 / 방법 |
|------|------------|
| 전체 정적 검증 | PowerShell: `.\scripts\validate-plugin.ps1`, Git Bash: `bash scripts/validate-plugin.sh` |
| 설치 표면 검증 | Claude+Codex: `.\scripts\validate-plugin.ps1 -Installed` 또는 `bash scripts/validate-plugin.sh --installed` / 개별 검증: `-ClaudeInstalled`, `-CodexInstalled`, `--claude-installed`, `--codex-installed` |
...
| 설치 동작 | `claude plugin marketplace add kingkingburger/plugin-mh` + `claude plugin install plugin-mh` (Claude), `bash codex/install.sh` (Codex) |
...
codex/               ← Codex 어댑터 (변환본)
  ...
  └── install.{ps1,sh}     ← ~/.codex/prompts/와 ~/.codex/skills/ 로 링크
guardrails/          ← 개인 엔지니어링 하네스 규칙
  ├── core.md              ← 공통 작업·검증·설계 기준
  ├── laws.md              ← 소프트웨어 공학 법칙 → 행동 규칙
  ├── languages/           ← TypeScript / Rust / Python 기본값
  └── workflows/           ← TDD / review 작업 기준
scripts/             ← 정적 검증 스크립트
...
- 형식: `<type>(<topic>): <한 줄 요약>` + 빈 줄 + 상세 내용
- 본문 설명은 한국어로 작성하고, 무엇을 왜 바꿨는지와 제외한 범위를 적는다.
...
| 함정 | 원인 | 해결 |
|------|------|------|
| 새 스킬이 Claude에서 안 보임 | `plugin.json` 버전 미bump 또는 Claude plugin cache stale | version bump 후 재설치, `validate-plugin --claude-installed`로 확인 |
| Codex에서 안 보임 | `~/.codex/prompts/` 또는 `~/.codex/skills/` 미링크 | `codex/install.sh` 또는 `.ps1` 실행 후 Codex 재시작, `validate-plugin --codex-installed`로 확인 |
...
| [codex/AGENTS.md](./codex/AGENTS.md) | Codex 27개 슬래시 커맨드 카탈로그 + 자연어 트리거 매핑 |
| [codex/template/AGENTS.md](./codex/template/AGENTS.md) | 다른 프로젝트에 배포 가능한 워크플로우 템플릿 |
| [guardrails/README.md](./guardrails/README.md) | 언어별 선호, TDD, 리뷰, 소프트웨어 공학 법칙 기반 하네스 규칙 |
...
- TypeScript / Rust / Python 작업 선호가 바뀌면 `guardrails/languages/` 에 먼저 반영

갱신 시 커밋 예시: `docs(agents): AGENTS.md 피드백 루프 갱신 - <함정 설명>` (피드백 루프 변경은 코드 변경이 아니므로 PATCH bump 불필요).
