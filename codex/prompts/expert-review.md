# /expert-review — Expert Review: 전문가 페르소나 병렬 리뷰 + 통합 제안

파일을 입력받아 최적의 전문가 페르소나를 자동 추천하고, 병렬 리뷰를 수행한 뒤 통합 제안서를 출력한다.
...
3. 파일이 없거나 읽기 실패 시 사용자에게 경로를 다시 확인한다.
...
`--auto`가 아닌 경우, 사용자에게 다음 질문을 제시하고 번호 또는 라벨로 답하게 하세요:

**다음 전문가 {N}명으로 리뷰를 진행할까요?**
1. **이대로 진행** — {페르소나1} / {페르소나2} / {페르소나3} (추천)
2. **일부 교체** — 교체하고 싶은 페르소나를 알려주세요
3. **전체 변경** — 원하는 전문가 역할을 직접 지정

"일부 교체" 또는 "전체 변경" 선택 시, 원하는 역할을 추가로 입력받는다.

### Step 4: 병렬 리뷰 실행

확정된 N명의 전문가 페르소나로 각각 독립 리뷰를 작성하라. **모든 N명의 리뷰를 한 응답에서 동시에 작성**한다.

각 전문가 리뷰에서 이 단계에서는 **{페르소나명}**처럼 사고하세요:
...
모든 리뷰가 수집되면, 다음 분석을 직접 수행하세요: 전문가 리뷰 통합자 관점으로 통합 제안서를 작성한다.

이 단계에서는 **전문가 리뷰 통합자**처럼 사고하세요:
...
---
## 참조 파일 위치
이 프롬프트가 언급하는 `references/...` 파일은 plugin-mh 저장소의 `skills/expert-review/references/` 에 있다.
