...
plugin-mh의 25개 스킬 + 2개 에이전트를 OpenAI Codex CLI 슬래시 커맨드로 변환해둔 어댑터.
루트의 `guardrails/` 는 TypeScript, Rust, Python, TDD, 리뷰, 소프트웨어 공학 법칙을 Codex 작업 규칙으로 제공한다.
...
| Codex CLI | `codex/prompts/` 와 `skills/` 를 `~/.codex/` 에 링크 | 슬래시 커맨드 27개 + 스킬 원본 참조 추가 |
...
기본 동작은 prompts는 심볼릭 링크, skills는 디렉터리 링크/junction 생성이다. 저장소를 업데이트하면 Codex 측 프롬프트와 스킬 원본 참조도 자동 반영된다.
...
Codex CLI에서 `/` 입력 시 27개의 plugin-mh 명령어가 보여야 한다.
...
```
codex/
├── AGENTS.md                ← 27개 명령어 카탈로그 + 자연어 트리거 매핑
├── README.md                ← 본 파일
├── install.ps1              ← Windows prompts + skills 설치 스크립트
├── install.sh               ← macOS/Linux prompts + skills 설치 스크립트
├── user-global-AGENTS.md    ← 개인 글로벌 가이드 (~/.codex/AGENTS.md 용)
├── template/
│   └── AGENTS.md            ← 다른 프로젝트에 배포 가능한 워크플로우 템플릿
└── prompts/                 ← 27개 슬래시 커맨드 프롬프트
    ├── agent-arena.md
    ├── ai-slop-cleaner.md
    ├── auto-commit.md
    ├── clarify.md
...
루트의 `guardrails/` 는 Codex가 프로젝트별 `AGENTS.md` 다음으로 참고할 수 있는 개인 하네스 규칙이다.
...
| **카탈로그** | [`./AGENTS.md`](./AGENTS.md) | 27개 슬래시 커맨드 카탈로그 + 자연어 트리거 매핑 |
...
| **가드레일** | [`../guardrails/README.md`](../guardrails/README.md) | 언어별 선호와 법칙 기반 작업 제약 |
